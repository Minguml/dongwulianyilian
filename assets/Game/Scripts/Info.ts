import DataCenter, {
	dc,
	field,
} from "../../framework/plugin_boosts/misc/DataCenter";
...
export default class UserInfoClass extends DataCenter {
	...
	onGetConfig(data) {
		...
		Platform.initBannerAd(
			this.getChoice(ChoiceType.BannerAdRefresh)
		);
	}
	...
	addDiamond(d, b = true) {
		...
		if (b) {
			Toast.make(
				I18Manager.t("earned_diamonds").replace(
					"%s",
					d.toString()
				)
			);
		...
		if (!this.firstTimeReach) {
			if (this.diamond >= 500) {
				Toast.make(I18Manager.t("can_buy_skins"), 2);
	...
	getSkinById(id: any): any {
		let res = R.skinConfig.json.filter((v) => {
			return v.id == id;
		});
		return res[0];
	}
